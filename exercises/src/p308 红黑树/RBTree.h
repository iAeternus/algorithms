...
public:
    // 颜色相关变量定义
    using Color = bool;
    constexpr static bool RED = true;    // 红
    constexpr static bool BLACK = false; // 黑
    ...
    struct stNode
    {
        ...
        stNode(const keyType &key, const dataType &data, Color color = RED,
        ...
        stNode(keyType &&key, dataType &&data, Color color = RED)
            : key(std::move(key)), data(std::move(data)), color(color), left(nullptr), right(nullptr), parent(nullptr)
        {}

        stNode(const stNode &other)
            : key(other.key), data(other.data), color(other.color), parent(other.parent), left(other.left), right(other.right)
        {}
...
protected:
    ...
    int size;     // 节点个数
    // 哨兵：所有叶结点的孩子且是根节点的父亲
    inline static node_t *nil = new node_t(std::numeric_limits<keyType>::max(), std::numeric_limits<dataType>::max(), BLACK);
